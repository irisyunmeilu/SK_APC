...
#supplemental analysis: using post adolescence data only#
...
#library(gtable);library(grid);library(gridExtra);library(xlsx)#save a dataset for supplemental analysis, post-adolescence
dt.5=subset(dt.5, age.n>17)
#write.csv(dt.f1, file=file.path(dataDir,"data_postad.csv"), row.names=FALSE)
...
age=seq(22,52,by=5) #post adolescence
cohort=seq(1930,1995,5)
for (o in 1:4){
  a=off.name[o]
  dt=subset(dt.5, c3==a)
  ...
  table1=data.frame(age.group=rep(c("20-24","25-29","30-34",
  ...
  #Table 2b-Interaction terms
  #exporting cohort coefficients into a table#
  table2b=data.frame(age.group=c("20-24","25-29","30-34",
...
ggsave("Figure2_713_app.png", path = figDir, age.plot, width = 7.5, height = 7.5, dpi = 300, bg = 'white')
ggsave("Figure2_713_app.eps", path = figDir, age.plot, width = 7.5, height = 7.5, dpi = 300, bg = 'white')


###############################################################################
#now figure 3: predicted curve by age main effects & APC
plot3.list=NULL
off.name1=names(table(dt.5$c3));off.name1[1]="Assault"; off.name1[3]="Ordinary Theft"
...
for (i in 1:4){
  ...
  plot3=ggplot(d1, aes(x=age, y=rate, group=Model, shape=Model,colour=Model))+
    ...
    scale_x_continuous(limits = c(20, 55),breaks = seq(20,55, by=10))+
...
ggsave("figure3_713_app.png",path = figDir, plot=plot3a,width=8.5,height=12, dpi = 300, bg = 'white')
ggsave("figure3_713_app.eps",path = figDir, plot=plot3a,width=8.5,height=12, dpi = 300, bg = 'white')
